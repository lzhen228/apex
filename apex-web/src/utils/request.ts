...
function isAuthEntryRequest(url?: string) {
  return !!url && (url.includes('/auth/login') || url.includes('/auth/feishu/login'))
}
...
request.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    if (token && config.headers && !isAuthEntryRequest(config.url)) {
...
request.interceptors.response.use(
  ...
  (error: AxiosError<Result>) => {
    ...
    if (response) {
      ...
      if ((status === 401 || status === 403) && !isAuthEntryRequest(error.config?.url)) {
        redirectToLogin()
      }
