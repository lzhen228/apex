...
import type {
  Result,
  CompetitionMatrixRequest,
  CompetitionMatrixData,
  CellDrugsData,
} from '@/types'
...
export async function getCompetitionMatrix(
  data: CompetitionMatrixRequest
): Promise<Result<CompetitionMatrixData>> {
  const response = await request.post<Result<CompetitionMatrixData>>('/competition/matrix', data)
  return response.data
}

export async function getCellDrugs(params: {
  target: string
  pairTarget: string
  diseaseIds: number[]
  phases?: string[]
}): Promise<Result<CellDrugsData>> {
